<?php
    ...
    if (strlen($body) > 0) {
        ...
        try {
            ...
            if (!empty($matches)) {
                echo json_encode(['matches' => $groupMatch, 'error' => sprintf('The user "%s" is already part of "%s" group', $_GET['name'], $json['name'])]);
                die();
            }

            $result = preg_replace(sprintf('/%s:.*/', $json['name']), sprintf('%s %s', $groupMatch[0], $_GET['name']), $groups);

            file_put_contents(sprintf('%s/groups', $apacheRoot), $result);
            echo json_encode(['message' => sprintf('The user "%s" was successfully added to the group "%s"', $_GET['name'], $json['name'])]);
            die();
        } catch (Exception $e) {
