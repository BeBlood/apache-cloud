<?php
    ...
    if (
        ...
        if ($name === 'admin' || $name === 'moderator') {
            echo json_encode(['error' => sprintf('The user %s can\'t be deleted', $name)]);
            die();
        }

        $apacheRoot = '/var/www/html';
        $documentRoot = sprintf('%s/public', $apacheRoot);
        $path = sprintf('%s/security_users/%s', $documentRoot, $name);

        if (!file_exists($path) || !is_dir($path)) {
            echo json_encode(['error' => sprintf('No dir exists for user %s', $name)]);
            die();
        }
        ...
        $result = preg_replace(sprintf('/ %s/', $name), '', $content);
        file_put_contents(sprintf('%s/groups', $apacheRoot), $result);

        $content = file_get_contents(sprintf('%s/users', $apacheRoot));
        $result = preg_replace(sprintf('/^.*(?:%s\:).*$(?:\r\n|\n)?/m', $name), '', $content);
        file_put_contents(sprintf('%s/users', $apacheRoot), $result);

        echo json_encode(['message' => sprintf('The user "%s" was successfully deleted!', $name)]);
        die();
    }
?>
